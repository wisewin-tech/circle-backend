...
public class UserPictureController extends BaseCotroller {
    ...
    public void getPictureByStatus(String status, Integer pageNo, Integer pageSize, HttpServletRequest request, HttpServletResponse response) {
        //验证参数
        ...
        Integer count = userPictureService.getPictureByStatusCount(status);
        List<UserPictureBO> pictureBOS = new ArrayList<UserPictureBO>();
        if (count >= 0) {
            pictureBOS = userPictureService.getPictureByStatus(status, queryInfo.getPageOffset(), queryInfo.getPageSize());
        }
